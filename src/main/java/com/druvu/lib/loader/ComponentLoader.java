 ...
 * {@code META-INF/services/com.druvu.lib.loader.ComponentFactory} (or via {@code module-info.java})
 * whose {@link ComponentFactory#type()} matches the requested target class. The factory's
 * {@link ComponentFactory#createComponent(Dependencies)} is called with the supplied dependencies.
 * Throws if more than one matching factory is found.
 * <h2>Tier 2 — ServiceLoader.Provider service file</h2>
 * <p>If no {@code ComponentFactory} is found in Tier 1, searches for a {@link ComponentFactory}
 * registered under {@code META-INF/services/java.util.ServiceLoader$Provider}. Implementors who
 * prefer to use the JDK provider file as the registration point can use this path while still
 * implementing {@link ComponentFactory} and gaining full {@link Dependencies} support.
 * <h2>Tier 3 — Direct ServiceLoader fallback (no Dependencies)</h2>
 * <p>If no {@code ComponentFactory} is found in either registry, falls back to
 * {@link ServiceLoader#load(Class) ServiceLoader.load(targetClass)}, discovering implementations
 * registered directly under the target type. This path requires no dependency on this library from
 * the implementing side. Dependencies are not available on this path.
 * <h2>Thread safety</h2>
	...
	private static <T> ComponentFactory<T> createComponentFactory(Class<T> targetClass) {
		...
		try {
			...
			return findInProviderRegistry(targetClass).orElseThrow(() -> {
				var notFound = new TargetClassNotFoundException("ComponentFactory providing a %s not found".formatted(targetClass.getName()));
				notFound.addSuppressed(e);
				return notFound;
			});
		}
