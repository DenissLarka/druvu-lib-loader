 ...
 *
 * <p>Extends {@link ServiceLoader.Provider} to align with the JDK's provider model. Implementors must provide two
 * methods:
 *
 * <ul>
 ...
 * this factory to also serve as a no-arg {@link ServiceLoader.Provider} for the fallback loading path in
...
public interface ComponentFactory<T> extends ServiceLoader.Provider<T> {
     ...
     *
     * <p>No-op by default. Override to perform cleanup such as closing connections, unregistering listeners, or
     * flushing buffers.
     *
     ...
     *
     * <p>Delegates to {@link #createComponent(Dependencies)} with an empty {@link Dependencies} instance. This
     * satisfies the {@link ServiceLoader.Provider} contract, allowing the factory to be discovered via the direct
     * {@code ServiceLoader.load(targetClass)} fallback path.
     *
     * @return a new component instance; must not return {@code null}
     */
    @Override
