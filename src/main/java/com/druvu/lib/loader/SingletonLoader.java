 ...
 * Therefore, creation method: {@link  SingletonLoader#init} and usage method: {@link  SingletonLoader#instance}
 *
...
public final class SingletonLoader {
	...
	/**
	 * Initialize a singleton instance with no dependencies.
	 *
	 * @param targetClass the class type to create
	 * @param <T> the type parameter
	 * @return the created singleton instance
	 * @throws IllegalStateException if singleton already initialized
	 */
	public static <T> T init(final Class<T> targetClass) {
		return init(targetClass, new Dependencies());
	}

	/**
	 * Initialize a singleton instance with the provided dependencies.
	 ...
	 * @throws IllegalStateException if singleton already initialized
	 */
	public static <T> T init(final Class<T> targetClass, Dependencies dependencies) {
		// Create a StableValue supplier that will lazily create the component
		Supplier<Object> stableSupplier = StableValue.supplier(
			() -> ComponentLoader.create(targetClass, dependencies)
		);
		...
		// The StableValue ensures thread-safe creation even if instance() is called concurrently
		return targetClass.cast(stableSupplier.get());
	}

	/**
	 * Retrieve a previously initialized singleton instance.
	 *
	 * @param targetClass the class type to retrieve
	 * @param <T> the type parameter
	 * @return the singleton instance
	 * @throws IllegalStateException if singleton isn't yet initialized via {@link #init}
	 */
	public static <T> T instance(final Class<T> targetClass) {
		Supplier<Object> supplier = SINGLETONS_REGISTER.get(targetClass);
		if (supplier == null) {
			throw new IllegalStateException("Singleton must be initialised before use: " + targetClass);
		}
